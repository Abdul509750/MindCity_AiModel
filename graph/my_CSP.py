...
class CSP:
    ...
    def binaryConstraints(self, assignment, current_Node, proposed_domain):
        ...
        # --- OPTIMIZATION: Dynamic Spatial Repulsion for Hospitals ---
        if proposed_domain == "Hospital":
            hospital_limit = self.graph.typeLimits["Hospital"]
            
            # Calculate the ideal area per hospital based on the grid's total mass
            ideal_area = (self.graph.rows * self.graph.cols) / max(1, hospital_limit)
            
            # The ideal Manhattan exclusion radius is roughly the square root of the territory.
            # We scale it by 0.8 to ensure the CSP doesn't mathematically gridlock itself.
            min_dist = max(3, int(math.sqrt(ideal_area) * 0.8)) 

            for (nr, nc), assigned_node in assignment.items():
                if assigned_node.NodeType == "Hospital":
